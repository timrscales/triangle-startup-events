// Triangle Startup Events — views (Month, Week, List) and filter bar.

const { useState: useS2, useMemo: useM2 } = React;

// ──────────────────────── Filter bar ────────────────────────
const FilterBar = ({ device, filters, setFilters, search, setSearch, searchOpen, setSearchOpen, resultCount, view, events }) => {
  const isMobile = device === "mobile";
  const tags = useM2(() => topTags(events, 8), [events]);
...
// ──────────────────────── Month grid ────────────────────────
const MonthView = ({ device, cursor, events, onSelectEvent, onSelectDay }) => {
  const isMobile = device === "mobile";
...
const MonthViewMobile = ({ cursor, events, days, onSelectEvent }) => {
  const [selectedDay, setSelectedDay] = useS2(TODAY);
...
// ──────────────────────── Week view ────────────────────────
const WeekView = ({ device, cursor, events, onSelectEvent }) => {
  const isMobile = device === "mobile";
...
// ──────────────────────── List view ────────────────────────
const ListView = ({ device, events, onSelectEvent, cardVariant }) => {
  const isMobile = device === "mobile";
  // Group by date
  const groups = {};
...
const EventCard = ({ event, variant = "standard", onClick }) => {
  const style = eventStyle(event);
...
Object.assign(window, { FilterBar, MonthView, WeekView, ListView, EventCard, TypeChip, PickBadge });
