// Triangle Startup Events — detail panel, submit modal, main app component.
...
// ──────────────────────── Period navigator (Month/Week prev/next + label) ────────────────────────
...
// ──────────────────────── Main app ────────────────────────
const TriangleEventsApp = ({ device = "desktop", cardVariant = "standard" }) => {
  ...
  // For list view, show upcoming only by default (today and later).
  const filteredAll = useM3(() => applyFilters(EVENTS, filters, search), [filters, search]);
  const filteredForView = useM3(() => {
...
Object.assign(window, { DetailPanel, SubmitModal, PeriodNav, Footer, TriangleEventsApp });