// Triangle Startup Events — interactive prototype
// Responsive: device prop ('desktop'|'mobile') determines layout density.

const { useState, useMemo, useEffect, useRef } = React;
const EVENTS = window.TRIANGLE_EVENTS;
const TYPE_STYLES = window.EVENT_TYPE_STYLES;
const TODAY = window.PROTO_TODAY;
...
// ──────────────────────── filter helpers ────────────────────────
const ALL_CITIES = ["Raleigh", "Durham", "Chapel Hill", "RTP"];
const ALL_TYPES = ["Talk", "Panel", "Workshop", "Happy Hour", "Networking", "Demo Day"];
const ALL_AUDIENCES = ["Founders", "Engineers", "Designers", "Investors"];
const ALL_TOPICS = ["AI", "fundraising", "hardware", "design", "networking", "happy hour"];

// ──────────────────────── tag palette (shared) ────────────────────────
// Single source of truth for the 5-accent palette used by both tag chips
...
// Style an event by its primary (first) topic tag, so chip color === pill color.
...
function applyFilters(events, f, search) {
...
// ──────────────────────── small UI atoms ────────────────────────
const TopBar = ({ device, view, setView, onSubmit, onSearch, searchOpen, setSearchOpen, savedCount }) => {
...
// Square cyan glyph: a stacked 3-block "triangle" hint
<div style={{
...
const ViewToggle = ({ view, setView, isMobile }) => {
...
Object.assign(window, { TopBar, ViewToggle, Logomark, iconBtn, ctaBtn, SearchIcon, PlusIcon, ChevronLeft, ChevronRight, XIcon, PinIcon, ExternalIcon, StarIcon,
  parseDate, sameDay, addDays, startOfWeek, startOfMonth, fmtTime, fmtTimeRange, durationHours,
  MONTHS, DOW_SHORT, DOW_FULL, ALL_CITIES, ALL_TYPES, ALL_AUDIENCES, ALL_TOPICS, applyFilters,
  TAG_PALETTE, ACCENT_PALETTE, hashIndex, eventStyle, tagStyle, topTags
});